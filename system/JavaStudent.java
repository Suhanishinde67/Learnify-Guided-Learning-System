...
public class JavaStudent extends JFrame {

    JPanel contentPanel;

    JavaStudent() {

        setTitle("Java Questions");
        ...
        JLabel heading = new JLabel("Important Java Questions");
    ...
    void loadQuestions() {
        ...
        try {
            ...
            ResultSet rs = c.statement.executeQuery(
                "SELECT * FROM java_questions ORDER BY id DESC"
            );

            while (rs.next()) {
                y = addBlock(rs.getString("question"), rs.getString("answer"), y);
            }
